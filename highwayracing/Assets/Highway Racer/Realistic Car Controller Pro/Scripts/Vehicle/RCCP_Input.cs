...
public class RCCP_Input : RCCP_Component {
    ...
    /// Internal inputs.
    /// </summary>
    public float steerInput_Centered = 0f;
    ...
    private void FixedUpdate() {
        ...
        if (inputs != null) {
            ...
            steerInput = inputs.steerInput + steerInput_Centered;
        ...
        angle = HR_CalculateAngle.CalculateAngle(transform.rotation, Quaternion.identity * Quaternion.AngleAxis(CarController.FrontAxle.maxSteerAngle * (steerInput * steeringCurve.Evaluate(CarController.speed)), Vector3.up), CarController.FrontAxle.maxSteerAngle * steeringCurve.Evaluate(CarController.speed));
    ...
    private void ExternalInputs() {
        ...
        if (CarController.Gearbox) {
            ...
            if (CarController.Gearbox.automaticTransmission && autoReverse) {
        ...
        if (CarController.Gearbox && !CarController.Gearbox.automaticTransmission)
    ...
    /// </summary>
    private void RCCP_InputManager_OnGearToggle() {
        ...
        //  Shifting down.
        CarController.Gearbox.automaticTransmission = !CarController.Gearbox.automaticTransmission;

        //  Informer.
        if (RCCP_Settings.Instance.useInputDebugger)
            RCCP_Events.Event_OnRCCPUIInformer("Automatic Gearbox = " + CarController.Gearbox.automaticTransmission.ToString());
